...
export function ThumbnailPicker({ file, previewUrl, onChange, disabled }) {
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs text-gray-400">
            {previewUrl ? (
              ...
              <>
                <span className="text-white font-medium">Drop JPG here</span> or{' '}
...
export function PreviewPicker({ previewUrls, onAddFiles, onRemove, disabled }) {
  const inputRef = useRef(null)
        ...
        <div className="mt-2 grid grid-cols-4 gap-2">
          {previewUrls.map((url, idx) => (
            <div key={idx} className="relative group">
              <img src={url} alt={`preview-${idx}`} className="w-full aspect-square object-cover rounded-lg" />
              <button
                type="button"
